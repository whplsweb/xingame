...
var KTModalCreateProjectSettings=function(){var e,t,i,o,r;return{init:function(){o=KTModalCreateProject.getForm(),r=KTModalCreateProject.getStepperObj(),e=KTModalCreateProject.getStepper().querySelector('[data-kt-element="settings-next"]'),t=KTModalCreateProject.getStepper().querySelector('[data-kt-element="settings-previous"]'),
        new Dropzone("#kt_modal_create_project_settings_logo",
        {
            ...
            renameFile: function (file) {
                var seed = "1234567890abcdef"
                var output  = ""
                seed=seed.split('');
                var len = 16
                while(len--){
                    var r=Math.floor(Math.random()*seed.length);
                    output+=seed[r];
                }
                output = output+'.'+file.name.split('.').pop()
                return output
            },
            accept:function(e,t){
